...
(function() {
  ...
  function setMeta(name, content, attr = 'name') {
    ...
    if (meta) {
      meta.content = (content === undefined || content === null) ? '' : content;
    }
  }
  ...
  function renderHero() {
    ...
    if (authorsEl) {
      authorsEl.innerHTML = authors.map((author, i) => {
        ...
        const corrSup = author.corresponding ? '<sup>†</sup>' : '';
        const comma = i < authors.length - 1 ? ',' : '';
        const nameHtml = author.link ? `<a href="${author.link}" target="_blank">${author.name}</a>` : `${author.name}`;
        return `<span class="author-block">
          ${nameHtml}${affSup}${eqSup}${corrSup}${comma}
        </span>`;
      }).join(' ');
    }
  ...
  function renderLinks() {
    ...
    const linkConfigs = [
      ...
      { key: 'slides', icon: 'fas fa-file-powerpoint', text: 'Slides' },
  ...
  function renderTeaserVideo() {
    ...
    const sectionEl = document.getElementById('teaser-section');
    if (!teaserVideo || !teaserVideo.src) {
  ...
  function renderSections() {
    ...
    container.innerHTML = sections.map((section, i) => {
      ...
      const sectionTitleNormalized = (section.title || '').trim().toLowerCase();
      const isOverview = i === 0 || sectionTitleNormalized === 'overview';
      const isTrainingPipeline = sectionTitleNormalized === 'training pipeline';
      const isExperiments = sectionTitleNormalized.startsWith('experiments:');
      const isImageFirst = isOverview || isTrainingPipeline;
      const titleClass = (isImageFirst || isExperiments) ? 'title is-3 has-text-centered' : 'title is-3';

      const imageSrc = section.image?.src || '';
      const imageAlt = section.image?.alt || '';
      const imageCaption = section.image?.caption || '';
      const sectionText = section.text || '';

      const captionHtml = imageCaption
        ? (isExperiments
          ? `<figcaption class="mt-4"><div class="content has-text-justified"><p>${imageCaption}</p></div></figcaption>`
          : `<figcaption class="has-text-centered mt-2">${imageCaption}</figcaption>`)
        : '';
            ...
            <div class="container is-max-desktop">
              <div class="columns is-centered">
                <div class="column is-four-fifths">
                  <h2 class="${titleClass}">${section.title}</h2>
                  <figure class="image section-figure">
                    <img src="${imageSrc}" alt="${imageAlt}" loading="lazy"/>
                    <figcaption class="mt-4">
                      <div class="content has-text-justified"><p>${section.text}</p></div>
                    </figcaption>
                  </figure>
                </div>
              </div>
        ...
        <section class="section hero ${isLight}">
          <div class="container is-max-desktop">
            <div class="columns is-centered">
              <div class="column is-four-fifths">
                <h2 class="${titleClass}">${section.title}</h2>
                ${sectionText ? `<div class="content has-text-justified"><p>${sectionText}</p></div>` : ''}
                <figure class="image section-figure">
                  <img src="${imageSrc}" alt="${imageAlt}" loading="lazy"/>
                  ${captionHtml}
                </figure>
